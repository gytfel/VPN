...
Чтобы одну подписку не расшаривали на весь подъезд, включите проверку HWID. В ветке 3.x это не переменная окружения, а настройка подписки — скрипт делает это через API:
...
`--limit` — сколько устройств разрешено тем, у кого лимит не задан персонально. Посмотреть состояние: `--status`, откатить: `--off`.

Как панель это считает:

| Что | Значение |
|---|---|
| `hwidDeviceLimit` у пользователя | главнее общего лимита |
| `hwidDeviceLimit = 0` | безлимит для этого пользователя, а не запрет |
| `hwidDeviceLimit` не задан | берётся общий `--limit` |

**Главная грабля.** После включения клиенты, которые не умеют HWID, перестают получать подписку вообще — панель отвечает отказом, а не молча пускает. Happ, v2rayNG и Streisand HWID шлют. Проверьте на живом пользователе, прежде чем включать для всех.

Привязанные устройства видны в панели: пользователь → Devices. Оттуда же удаляются, когда человек меняет телефон.
